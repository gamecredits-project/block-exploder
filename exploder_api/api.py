...
def get_block_confirmations(block_hash):
    ...
    return {
        "hash": block_hash,
        "confirmations": db.calculate_block_confirmations(rpc, block)
    }
...
def get_transaction_by_txid(txid):
    ...
    try:
        return TransactionSerializer.to_web(db.get_transaction_by_txid(rpc, txid))
    except KeyError:
...
def get_transaction_confirmations(txid):
    if not validate_sha256_hash(txid):
        return "Invalid transaction ID", 400
    try:
        tr = db.get_transaction_by_txid(rpc, txid)
    except KeyError:
